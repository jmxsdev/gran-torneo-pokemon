...
|---|---|---|---|---|
| **D1** | Fórmula de daño | `base = (2*nivel/5 + 2) * ataque / defensa + 2` (enteros, en ese orden); `mult = tipos_multiplicador(tipo_primario_del_atacante, tipo1_def, tipo2_def)`; `daño = (int)(base * mult)`; si `mult > 0` y `daño < 1` ⇒ `daño = 1`; si `mult == 0` ⇒ `daño = 0` | Fórmula oficial 1.ª gen completa `/50` (descartada, ver abajo); con poder de movimiento (fuera de alcance) | Usa el mínimo exigido (nivel, ataque, defensa, tipo atacante, tipo(s) defensor — RF-CMB-03). **Nota clave**: se elimina el `/50` de la fórmula oficial porque ese término acompaña al poder del movimiento, y los movimientos están fuera de alcance; sin él, el daño a nivel 50 contra stats derivadas de base 45–80 es ≈2 por golpe y ningún combate terminaría antes del límite de turnos (todo sería empate). Con la variante cerrada: daño ≈ 24 vs HP ≈ 95 (4 golpes), ×2 ⇒ 2 golpes, ×0.5 ⇒ 8: combates de ~6–16 turnos, dentro del límite de 20. Multiplicador del **tipo primario** del atacante: al no existir movimientos, el tipo primario es el identificador natural de la especie. Mínimo 1 si hay efecto: evita el estancamiento total |
| **D2** | Stats del ejemplar | `stat = stat_base + nivel` para las cuatro stats (hp, ataque, defensa, velocidad); `hp_actual = hp_max` al crear y al iniciar cada combate; **sin componente aleatorio** | Fórmula con variación ±15 % y `srand(fija)` (exploración); fórmulas oficiales con IVs | Dos ejemplares de la misma especie con **niveles distintos** difieren (escenario del spec RF-EQP-03 usa exactamente ese caso); a igual nivel son iguales, lo que el spec permite ("pueden diferir"). La ausencia total de azar hace la batería `diff` 100 % reproducible sin gestionar semillas, y la fórmula se explica en una línea en la oral |
...
| **D6** | Tamaño de equipo | `MAX_EQUIPO = 6` (rango válido de inscripción 1–6, RF-EQP-05); `TAM_EQUIPO_TORNEO = 3` para los equipos que combaten en el torneo (el spec fija 3); el backtracking acepta cualquier `n` en 1..`MAX_EQUIPO` | Equipo fijo de 6 para todo; equipo fijo de 3 sin rango | La constante `MAX_EQUIPO` acota arreglos y validaciones; separar el tamaño del torneo en otra constante deja ambos valores cambiables en una línea si el docente lo define (ver Preguntas abiertas #2) |
| **D7** | Rango de niveles | `NIVEL_MIN = 1`, `NIVEL_MAX = 50` (alineado al spec vigente: RF-EQP-05 rechaza 0 y 51; ejemplos del PDF: niveles 12 y 15) | 1–100 (indicación posterior del orquestador) | El spec es el contrato de verificación y rechaza explícitamente el nivel 51; el rango queda parametrizado en `constantes.h`: si el docente confirma 1–100, el cambio es una constante **más** un delta de spec (ver Preguntas abiertas #1). Niveles bajos mantienen combates cortos y stats coherentes con bases 45–80 |
...
|---|---|---|
| `src/constantes.h` | Crear | Constantes de dominio (`MAX_EQUIPO=6`, `TAM_EQUIPO_TORNEO=3`, `NIVEL_MIN/NIVEL_MAX`, `MAX_ENTRENADORES=32`, `POKEDEX_MAX=150`, `TOTAL_COMBATES=64`, `MAX_TURNOS_COMBATE=20`, buffers) |
...
| E2E (scriptada) | Rutas inválidas: opción 99, nivel 51, id duplicado, resultado de eliminado, empate en 49–64, `pokedex.txt` ausente | Casos con solo entradas inválidas: se verifica que el programa sigue vivo (sigue mostrando el menú en el stdout esperado) |
...
- [ ] **Rango de niveles**: el spec vigente fija 1–50 y rechaza el nivel 51 (RF-EQP-05); la misión de diseño indicó 1–100. El diseño queda parametrizado (`NIVEL_MAX`): si el docente confirma 1–100, se requiere cambiar una constante **y** un delta de spec (MODIFIED en RF-EQP-05 y su escenario) antes de `sdd-apply`.
- [ ] **Tamaño del equipo de torneo**: el spec fija 3 (`TAM_EQUIPO_TORNEO`) con rango válido 1–6 (`MAX_EQUIPO`); confirmar con el docente si el torneo debe usar 6 (cambio de una constante + delta menor de spec).
- [ ] **D9**: enfrentamiento directo + id menor como criterio adicional de desempate — confirmar con el docente (el PDF lo remite explícitamente a él); documentar la confirmación en `docs/informe-tecnico.md`.
